-- pvp-v7-hotfix.sql
-- Fix: pvp-v7-fixes.sql referenced pvp_shuffle() which doesn't exist
-- in this project. pvp-v6.sql's pvp_deal inlined the shuffle via
-- `v_deck := ARRAY(SELECT r||s ... ORDER BY random())`. Restore that
-- inline shuffle while keeping the v7 stale-threshold change (60s).
...
DECLARE
    ...
    v_stale_threshold DOUBLE PRECISION := 60.0;   -- v7: was 15.0
    v_hand_num INTEGER;
    v_deck TEXT[];
    v_folded BOOLEAN[];
    v_sb_seat INTEGER;
...
BEGIN
    ...
    -- ========== INLINE SHUFFLE (as in pvp-v6.sql) ==========
    v_deck := ARRAY(
        SELECT r || s FROM
