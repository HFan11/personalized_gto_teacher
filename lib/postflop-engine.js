...
const { computeEquityBuckets, getHandBucket, handConflictsWithBoard, handToCanonical, makeCard } = require('./hand-utils');
...
class PostflopSolver {
    ...
    solve(options = {}) {
        ...
        // Step 6: Run CFR+ with sampling cap for O(1) per-iteration cost
        const totalPairs = oopBucketIds.length * ipBucketIds.length;
        const samplesPerIter = totalPairs <= 1024 ? 0 : Math.floor(totalPairs * 0.8);
        const boardSeed = typeof hashSeed === 'function'
            ? hashSeed(this.board.map(c => c.id).join(',')) : 42;
    ...
    _buildFacingBetNode(player, pot, remainingStack, betToCall, pathPrefix) {
        ...
        // Fold → opponent wins pot. Payoff = ±halfOriginalPot
        const halfOrigPot = (pot - betToCall) / 2;
        const foldNode = new GameNode(NodeType.TERMINAL, -1, [], pot, null);
        foldNode.payoffs = (h0, h1) => {
            if (player === 0) return [-halfOrigPot, halfOrigPot];
            return [halfOrigPot, -halfOrigPot];
        };
    ...
    _makeShowdownPayoffs(pot, ipBonus) {
        ...
        return (bucketOOP, bucketIP) => {
            ...
            return [pot * oopShare - halfPot, pot * (1 - oopShare) - halfPot];
        };
    }
...
function solvePostflopSpot(config) {
    const solver = new PostflopSolver({
        ...
        simsPerHand: config.simsPerHand || 80,
    });
...
module.exports = { PostflopSolver, solvePostflopSpot };
