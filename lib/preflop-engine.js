...
const { CFRSolver, GameNode, NodeType } = require('./cfr-engine');
const { generate169Hands } = require('./hand-utils');

// ============================================================
// Heads-Up Preflop Equity Model (same as web/js/preflop-solver.js)
// ============================================================
...
function _computeHeadsUpEquity(h0, h1) {
    ...
    if (isPair0 && isPair1) { return hi0 > hi1 ? 0.82 : 0.18; }
    if (isPair0 || isPair1) {
        let pRank, npHi, npLo, npSuited, pairFirst;
        if (isPair0) { pRank=hi0; npHi=hi1; npLo=lo1; npSuited=suited1; pairFirst=true; }
        else { pRank=hi1; npHi=hi0; npLo=lo0; npSuited=suited0; pairFirst=false; }
    ...
    if (hi0===hi1) {
        if (lo0===lo1) { if (suited0&&!suited1) return 0.53; if (!suited0&&suited1) return 0.47; return 0.5; }
        let eq = lo0>lo1 ? 0.72 : 0.28;
        if (suited0) eq+=0.02; if (suited1) eq-=0.02;
        return Math.max(0.15,Math.min(0.85,eq));
    }
    if (lo0===lo1) { let eq=hi0>hi1?0.70:0.30; if(suited0)eq+=0.02;if(suited1)eq-=0.02; return Math.max(0.15,Math.min(0.85,eq)); }
    if (hi0===lo1) { let eq=0.28; if(suited0)eq+=0.03;if(suited1)eq-=0.03; return Math.max(0.15,Math.min(0.85,eq)); }
    if (lo0===hi1) { let eq=0.72; if(suited0)eq+=0.03;if(suited1)eq-=0.03; return Math.max(0.15,Math.min(0.85,eq)); }
    const eq0=_PREFLOP_EQUITY_TABLE[h0]||0.45, eq1=_PREFLOP_EQUITY_TABLE[h1]||0.45;
    const diff=eq0-eq1;
...
function _showdownPayoffIP(h0, h1, halfPot, hands169) {
    ...
    let gap = 99;
    if (hand && hand.length === 3) { gap = Math.abs(_rv[hand[0]] - _rv[hand[1]]); }
    const connected = gap <= 2;
    let bonus = 0.06;
    if (suited) bonus += 0.04;
    if (connected && !isPair) bonus += 0.03;
    if (isPair) bonus += 0.03;
...
class PreflopSolver {
    ...
    _calibrateRFI() {
        ...
        for (const pos of this.positions) {
            ...
            for (let i = 0; i < hands.length; i++) {
                const h = hands[i];
                if (i < target - 3) { this.strategies.set(h.key, [0.02, 0.98]); }
                else if (i < target + 3) {
                    const t = (i - (target - 3)) / 6;
                    const raiseF = Math.max(0.05, 1 - t * 0.9);
                    this.strategies.set(h.key, [1 - raiseF, raiseF]);
                } else { this.strategies.set(h.key, [0.98, 0.02]); }
            }
        }
    }
    ...
    _solveVs4BetScenarios(iterations) {
        ...
        callNode.payoffs = (h0, h1) => _showdownPayoff(h0, h1, pot);
